...
function registerIcons(): void {
  ...
  for (const [name, path] of iconsToLoad) {
    ...
    iconRegistry.addSvgIconInNamespace(NAMESPACE, name, sanitizer.bypassSecurityTrustResourceUrl(url.href), {withCredentials: true});
  }
}
...
function registerTestIcons(): void {
  ...
  for (const name of iconsToLoad.keys()) {
    iconRegistry.addSvgIconLiteralInNamespace(NAMESPACE, name, sanitizer.bypassSecurityTrustHtml(staticIcon));
  }
}

export const provideAppIcons = () => provideAppInitializer(registerIcons)

export const provideTestAppIcons = () => provideAppInitializer(registerTestIcons)
